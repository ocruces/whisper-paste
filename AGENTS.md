...
Three more invariants on the tray and hotkey:

- **`_register_hotkey` never raises.** It runs from `main()` before the tray icon exists in a process that may have no console (a raise means the user sees literally nothing), and from `on_resume()` on the power-monitor daemon thread (a raise leaves the app alive but deaf). It tries to register `config.HOTKEY`, falls back to `config.DEFAULT_HOTKEY` if that fails, and finally accepts no hotkey at all. After it returns, `config.HOTKEY` equals exactly what is actually registered, or is `None` if nothing is, so the tray tooltip cannot lie about the hotkey.

- **`_idle_title` goes through `_fit_tooltip`.** It used to be a hard-coded constant and fit by inspection; now `config.HOTKEY` is user-supplied from the ini or `--hotkey`, and a long value would overflow the fixed-size `szTip` field.

- **`_validate_hotkey` rejects any combination containing the Windows key.** This is not a theoretical concern — it is an empirical result, confirmed on hardware on 2026-08-01: a `win+...` hotkey does not work, because Windows handles those combinations above the `WH_KEYBOARD_LL` hook this app installs. Nothing in the `keyboard` library signals the problem — `keyboard.parse_hotkey` accepts the combination and `keyboard.add_hotkey` reports success — the hotkey simply never fires. That is why the check exists at all: registration succeeding is not evidence the hotkey works, so `_validate_hotkey` fails it closed before `_register_hotkey` ever tries. It flattens all three levels of `parse_hotkey`'s return value (steps of alternative-key-sets of scan-codes) into one set of used codes, so a Windows key buried in a later step of a comma-separated sequence (`a, win+b`) is caught too, not just a Windows key in the first step. It also derives the codes to check against from `keyboard.key_to_scan_codes("windows")` rather than hard-coding `57435, 57436`, so it tracks whatever the installed `keyboard` version resolves that name to instead of drifting from it. Do not relax this guard because the combination "parses fine" — parsing was never the problem.
...
CPU model resolution is strict: `_resolve_faster_whisper_model()` checks a
bundled directory, an explicit existing local directory, and then the
manifest-backed cache in `model_cache.py`. Only names in
`whisper_paste/resources/models.json` can trigger a pinned Hugging Face
download; unknown names and repo IDs fail before `WhisperModel` is constructed.
The managed cache lives under `%LOCALAPPDATA%\WhisperPaste\models` (falling
back to `~\WhisperPaste\models`) and verifies hashes only when publishing a
download or replacement. Later starts check required files without rehashing
the model weights.
...
The bundled model lives at `<exe dir>\models\<config.WHISPER_MODEL>` and is resolved by `transcriber._resolve_faster_whisper_model()` **at load time inside `_get_model()`**, never at import time — same config-mutation rule as everything else (see above): `main()` rewrites `config.WHISPER_MODEL` from `--model` after `transcriber` has been imported. Handing `WhisperModel` a directory is legitimate; a bundled or explicit local directory therefore short-circuits HuggingFace. An unbundled name is resolved through the manifest-backed cache, so unknown names and repo IDs never reach an unpinned downloader. The separator guard in `bundled_model_dir` does double duty: it lets explicit local paths continue to the local-directory check while repo IDs are handled by the manifest policy, **and** it stops `--model ..\..\x` from aiming the lookup outside the bundle. It also rejects a bare drive prefix like `C:x` via `os.path.splitdrive` — `os.path.isabs` calls that relative and there is no separator to catch it, but it is drive-relative, and `ntpath.join` collapses it (`ntpath.join(r'D:\app', 'models', 'C:x')` is `'C:x'`), so on any install drive other than `C:` it would otherwise resolve against the current directory on drive `C:` instead of the bundle. Keying on the bare name is also what makes "drop your own converted model into `models\my-model\` and pass `--model my-model`" work, which the README documents — don't narrow it to a hard-coded allow-list of model names.
...
`_label()` appends `— logging transcripts` to every tray title while `config.LOG_TRANSCRIPTS` is on. That exists because the settings file made the flag **durable**: it used to require `--log-transcripts` on each launch, so it could not outlive the session, and now it can outlive a reboot with nothing on screen to say so. `_label()` is the single chokepoint every tooltip is built from, which is why the marker lives there and not at each call site. Keep it plain BMP text — `_fit_tooltip` measures in UTF-16 code units and an emoji costs two of the 127 that error tooltips already spend on unbounded exception text.
...
**The spec has two mandatory data directives: `collect_data_files('faster_whisper')` and the explicit `whisper_paste/resources/models.json` resource.** `faster_whisper/vad.py` computes the Silero VAD path from its own `__file__`, which static analysis cannot see; without the directive there is no `faster_whisper/assets/silero_vad_v6.onnx` and the first real dictation dies with `NoSuchFile` — `transcribe()` passes `vad_filter=True` on every faster-whisper call, so that is the hot path, not an edge case. The explicit manifest resource keeps the frozen package and source install on the same verified model policy. Everything else that looks necessary is already done by PyInstaller's own hooks and hooks-contrib (`onnxruntime`, `sounddevice`/PortAudio, `pystray._win32`, `av`, numpy, PIL); adding manual `collect_*` calls for those produces doubled DLLs and a second place to go stale, so **don't**. `ctranslate2` needs no runtime hook either: `importlib.resources.files()` resolves correctly under PyInstaller 6.x because the `.pyd` sits beside the DLLs in the package directory. That was verified empirically rather than reasoned about, and is worth re-verifying on a dependency bump. The one deliberate piece of insurance is `collect_submodules('huggingface_hub')` — it is a lazy-loading package whose import graph shows almost nothing, and it is reached only by the manifest-limited download path that no unit test exercises.
